...
def heisenberg_trotter(couplings, p, time, depth):
    ...
    for i in range(4):
        if i != 3:
            ...
            j = 0
        coeffs.append(-couplings[0])
        observables.append(qml.PauliX(wires=[i]) @ qml.PauliX(wires=[j]))

        coeffs.append(-couplings[1])
        observables.append(qml.PauliY(wires=[i]) @ qml.PauliY(wires=[j]))

        coeffs.append(-couplings[2])
        observables.append(qml.PauliZ(wires=[i]) @ qml.PauliZ(wires=[j]))

    for k in range(4):
        coeffs.append(-couplings[3])
