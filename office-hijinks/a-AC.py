...
def time(hour, minute):
    ...
    hour_theta = -1 * (np.pi / 6) * (hour + minute / 60)
    minute_theta = -1 * (np.pi / 30) * minute

    qml.RY(phi=hour_theta, wires="hour")
    qml.RY(phi=minute_theta, wires="minute")
...
test_cases = [
    ...
    ["[1, 30]", "[0.0, 0.85355339, 0.0, 1.46446609e-01]"],
]
